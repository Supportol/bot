...
│   ├── telegram_messages.py   # разбиение длинных ответов для Telegram
│   ├── text.py                # /text <ID,...> — извлечение + рерайт + экспорт
│   ├── ixbt.py                # Внутренний хендлер источника iXBT (не в меню)
│   ├── drom.py                # Внутренний хендлер источника Drom (не в меню)
│   ├── motor.py               # Внутренний хендлер источника Motor (не в меню)
│   ├── images.py              # Переиспользуемая логика обработки обложек
│   ├── list.py                # Внутренний хендлер списка (не в меню)
│   ├── processing.py          # Внутренний хендлер извлечения (не в меню)
│   └── rewrite.py             # Legacy-обработчик /rewrite (не подключён)
├── services/
...
| Команда | Описание |
|---------|----------|
| `/news` | Сводная команда: **последовательно** запускает `iXBT → Drom → Motor`, сохраняет новые публикации и возвращает ID. Для новых записей автоматически запускает обработку обложек (эквивалент `/images <ID,...>`). При отсутствии новых — последние 5 из БД. Длинный ответ разбивается на части по 4000 символов. |
| `/text` | Единая команда по ID: всегда извлекает полный текст, обновляет БД, копирует обработанное изображение, создаёт `source.md`, запускает рерайт через TEXT.ru и сохраняет `news.md` в `news/ДД.ММ.ГГГГ/<ID>/`. Если ID не переданы — команда ждёт следующее сообщение с ID. |
...
- `parse_publication_ids(text)` — извлечение ID из аргументов команды (`/text`)
...
- Обложка: для каждой статьи загружается HTML страницы новости; приоритет — **первая кликабельная картинка** (`a[href]` на полноразмерный `.jpg` в `.b-left-side` / `div.news_img`, без превью `gen*` / `tn_`), иначе главная (`og:image` или `.b-media-cont img`), иначе превью из карточки ленты → `images/DR_HON_N/`
...
```json
{
  "image_processing": {
    ...
    "height": 500,
    "quality": 85,
    "watermark_max_width_ratio": 0.22
  }
}
```
...
- добавляет watermark из `watermark.png` в правый нижний угол
  ...
  - максимальный размер: `watermark_max_width_ratio` от ширины/высоты холста (по умолчанию `0.22`)
...
- `/news`, `/ixbt`, `/drom`, `/motor` после сохранения новых публикаций вызывают эту же обработку автоматически для полученных ID
...
- отправляет текст в TEXT.ru API (`services/rewrite_service.py`):
  ...
  - берёт заголовок из `source.md` (строка `# ...`), добавляет его в начало текста статьи и отправляет **одну** задачу рерайта в TEXT.ru
  - из переписанного результата извлекает заголовок из начала текста (первый абзац или первое предложение), остальное — тело статьи
  - опрашивает статус задачи до готового результата
- сохраняет результат в `news.md` в директории новости `news/ДД.ММ.ГГГГ/<ID>/`
  - строка `1. Заголовок:` — переписанный через TEXT.ru заголовок из `source.md` (строка `# ...`)
  - строка `2. Картинка:` — `<p><img src="/_upload/content/news/<имя-файла>.jpg" alt="Заголовок" /></p>` (имя файла — скопированная обложка из папки публикации)
  - далее текст рерайта: каждый абзац обёрнут в `<p>...</p>`
...
1. `/news` — собрать новые публикации, получить ID (`IX_HON_1`, `MT_AC_2`, …) и автоматически обработанные обложки
2. `/text IX_HON_1, MT_HON_3` — извлечь текст, выполнить рерайт и получить `source.md` + `news.md` в `news/.../<ID>/`
