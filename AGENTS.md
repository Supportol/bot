...
| Команда | Описание |
|---------|----------|
| `/news` | Сводная команда: **последовательно** запускает `iXBT → Drom → Motor`, сохраняет новые публикации и возвращает ID. Для новых записей автоматически запускает обработку обложек (эквивалент `/images <ID,...>`). При отсутствии новых — последние 5 из БД. |
